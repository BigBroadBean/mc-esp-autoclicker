...
- **自瞄**：自瞄开关 / 触发模式 / 自瞄键、目标过滤、优先级、FOV、距离、
  平滑度、反应延迟、鼠标灵敏度、预判 tick、切换冷却、第二目标、
  二段平滑、稳定度、视线检测、自瞄可视化
- **系统**：配置方案、方案循环热键
...
- **快捷键**：自瞄键由渲染线程热键路径同步（与 Insert/F8 同一套检测），
  支持按住与切换两种模式，切换时有 Toast 提示；总开关未打开时第一次按
  自瞄键会自动启用。
...
- `[clickerProfile1]`..`[clickerProfile4]`：4 套连点器方案槽，含
  `inGameGate`（游戏内门控）。
- `[aim]`：自瞄开关、触发模式与热键、目标过滤、优先级、FOV、距离、平滑度、
  反应延迟、鼠标灵敏度、预判 tick、切换冷却、第二目标、二段平滑、稳定度、
  可视化与视线检测。
- `[colors]`：ESP / 自瞄颜色。
