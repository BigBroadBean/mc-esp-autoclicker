...
> 自瞄已重新内置（碰撞箱锁定 + 人类化鼠标移动）；已移除：多倍点击（multi-click）、
> 滚轮点击（scroll-to-click）、外部渲染共享内存（esp_shm）与外部消息传递。
...
- **自瞄**：自瞄开关 / 自瞄开关热键 / 触发模式 / 自瞄键、目标过滤、优先级、
  FOV、距离、平滑度、辅助强度、反应延迟、速度微调、预判 tick、切换冷却、
  瞄准高度、第二目标、二段平滑、稳定度、黏锁、视线检测、自瞄可视化
- **系统**：配置方案、方案循环热键
...
| 循环切换 4 套连点方案 | `F8` |
| 自瞄总开关 | `F9` |
| 自瞄键（按住/切换模式） | 鼠标侧键 1（`VK_XBUTTON1`） |

通过快捷键开关 ESP / 连点器 / 门控 / 切换方案时，右下角会显示 Toast
（约 2.2 秒）。

游戏内右下角状态栏（HUD）**常态化显示**：进入游戏后无需开启 ESP / 连点器 /
自瞄也会常驻，不再依赖任何模式开关；打开菜单时改由菜单底部状态行显示。
内容包含 `ESP / 连点(实时 CPS) / 自瞄 / 攻击 / 放置 / 游戏内外 / 方案`，
状态变化时自动刷新。
...
- **触发模式**：按住左键 / 按住右键 / 按住自瞄键 / 自瞄键切换 / 始终瞄准，
  默认按住左键。松开触发立即停止移动；约 350ms 内重新触发仍可保留目标选择，
  但控制器会重置并重新执行反应延迟，不会携带旧速度或亚计数。
- **碰撞箱策略**：
  - 以碰撞箱内固定相对高度（`aimHeight` 45..80%，默认胸口）作为获取落点，
    不追随棱角，自己跳跃时也不会把准星抬向滑动边缘；
  - 进入碰撞箱后记录真实进盒点，再按第二目标过渡到水平中心 / 进盒高度 /
    保持进盒点 / 自定义高度，目标移动时落点随碰撞箱一起更新；
  - **二段平滑**使用真实时间，不依赖渲染帧率；**稳定度**先收敛到约 1px
    或半个最小鼠标计数，再以配置半径作为释放阈值，避免在目标内部反复抢鼠标。
- **角度域磁铁移动**：候选、FOV 和误差均按角度计算；独立 250Hz 线程按实际时间
  做加减速和亚鼠标计数累积，再通过 `SendInput` 发送。自动读取 Minecraft
  `Options.sensitivity`，所以分辨率、游戏 FOV 和帧率变化不会改变基本手感；
  进盒/释放 FOV 边缘自动降权。
- **辅助强度**（0..10）：0 只选择和显示目标不移动鼠标，6 默认磁吸，10 接近强纠。
- **黏锁**：当前目标只有在明显更差时才切换；短暂越过释放 FOV 或被墙挡住时保留
  目标但停止移动，宽限结束后必须重新进入获取 FOV 才能重新获取。
- **快捷键**：`自瞄总开关热键`（默认 `F9`）与 ESP 开关热键同级，按一下直接
  开/关并写回 `esp.ini`；`自瞄键`由渲染线程热键路径同步（与 Insert/F8
  同一套检测），支持按住与切换两种模式，切换时有 Toast 提示；总开关未打开时
  第一次按自瞄键会自动启用。
- **目标选择**：玩家 / 生物 / 其他实体过滤；准星最近 / 距离最近 / 血量最低三种优先级；
  获取 FOV 与约 1.18x 的释放 FOV 分离，边缘平滑降权，目标切换带显著性阈值。
- **反透视**：默认只获取视线可达目标（每帧最多检测 8 个候选）；当前目标被墙挡住
  有 180ms 连续性宽限，但宽限期间辅助倍率为 0，不会穿墙移动。
- **预判**：水平速度做轻量 EMA 后外推 0..20 tick；近战（<4 格）强制关闭，竖直不加提前量。
- **可视化**：可选目标点 / 准星到目标瞄准线 / 屏幕中心 FOV 圈，
  并在目标旁显示名字与血量；默认关闭，HUD 仍显示待机 / 搜索 / 跟随 / 进盒状态。
- 菜单打开、系统光标可见或游戏未在前台时自动暂停自瞄。
...
- 常态 HUD 下 SwapBuffers 每帧只读取相机（维持 HUD 与 GUI 显隐判断）；
  ESP / 自瞄 / 连点器门控全关时不采集实体，仍无逐实体 JNI 开销。
- 门控状态只在“连点器运行且相关门控开启”时每 5ms 读一次 JNI。
- 实体预投影数据从游戏线程到绘制线程改用 `swap`，不再每帧复制两个 `vector`。
- 宿主线程动态频率：菜单打开用 `renderHz`（默认 120），ESP / HUD / 自瞄可视化
  显示用 60Hz，全部关闭时降到 30Hz。
- 自瞄鼠标移动由独立 250Hz 线程执行，不占用游戏渲染线程；实体生命值/速度仅在
  自瞄触发时额外读取。
...
├── src/
│   ├── aimbot.h/.cpp        自瞄：碰撞箱策略 + 人类化鼠标移动线程
...
不存在时自动生成默认配置。
默认：**ESP 关闭**、**左右键连点开启**、**连点器总开关关闭**、**自瞄关闭**。
自瞄默认采用 70° 获取 FOV、32 格、平滑度 5、辅助强度 6、胸口高度，
进入碰撞箱后过渡到自定义高度中心；可视化默认关闭。
...
- `[clickerProfile1]`..`[clickerProfile4]`：4 套连点器方案槽，含
  `inGameGate`（游戏内门控）。
- `[aim]`：自瞄开关、总开关热键、触发模式与热键、目标过滤、优先级、FOV、
  距离、平滑度、反应延迟、速度微调、预判 tick、切换冷却、第二目标、
  二段平滑、稳定度、辅助强度、瞄准高度、黏锁、可视化与视线检测。
- `[colors]`：ESP / 自瞄颜色。
...
- 自瞄平滑度：1..10（角度域响应）
- 自瞄反应延迟：0..300 ms
- 自瞄速度微调：0.5..2.0（游戏灵敏度自动读取）
- 自瞄预判：0..20 tick
- 自瞄二段平滑：1..10
- 自瞄稳定度：0..30 px（释放半径）
- 自瞄辅助强度：0..10
- 自瞄瞄准高度：45..80（碰撞箱底部向上百分比）
