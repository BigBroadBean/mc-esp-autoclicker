...
- **自瞄**：自瞄开关 / 自瞄开关热键 / 触发模式 / 自瞄键、目标过滤、优先级、
  FOV、距离、平滑度、反应延迟、鼠标灵敏度、预判 tick、切换冷却、第二目标、
  二段平滑、稳定度、视线检测、自瞄可视化
- **系统**：配置方案、方案循环热键
...
- **触发模式**：按住左键 / 按住右键 / 按住自瞄键 / 自瞄键切换 / 始终瞄准，
  默认按住左键。
- **碰撞箱策略**：
  - 第一目标：准星未命中碰撞箱 → 瞄准离准星射线**最近的碰撞箱表面点**，把视野拉进碰撞箱；
  - 第二目标（可配置）：准星命中碰撞箱后切换落点——
    `放平中心`（本地玩家视平线高度的水平中心）/
    `不放平中心`（保持第一目标点高度的水平中心）/
    `保持最近点`（不切换）；
  - **二段平滑**：从第一目标到第二目标使用 smoothstep 过渡（1..10 可调），
    锁到碰撞箱瞬间不会突然跳向中心；
  - **稳定度**：第一次对齐第二目标后建立稳定锚点，目标只移动死区内的像素时
    不再移动鼠标（0..30 px 可调），超过死区才重新追瞄。
- **人类化鼠标移动**：独立 250Hz 线程 `SendInput` 相对移动；锁定新目标有反应延迟，
  启动逐渐加速、接近目标减速、带轻微噪声与偶发微停顿，不会瞬间锁死。
...
- **目标选择**：玩家 / 生物 / 其他实体过滤；准星最近 / 距离最近 / 血量最低三种优先级；
  FOV 角度范围与最大距离限制。
- **反透视**：默认只瞄准视线可达目标（墙壁后自动跳过，每帧最多检测 6 个候选）。
- **预判**：可按目标当前速度外推 0..20 tick。
- **可视化**：目标点 / 准星到目标瞄准线 / 屏幕中心 FOV 圈，
  并在目标旁显示名字与血量。
...
- `[aim]`：自瞄开关、总开关热键、触发模式与热键、目标过滤、优先级、FOV、
  距离、平滑度、反应延迟、鼠标灵敏度、预判 tick、切换冷却、第二目标、
  二段平滑、稳定度、可视化与视线检测。
- `[colors]`：ESP / 自瞄颜色。
